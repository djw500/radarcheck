//! Parity tests: verify Rust GRIB decode matches Python (cfgrib) output.
//!
//! Tests against fixture files in `tests/fixtures/grib_parity/`.
//! Each fixture has:
//!   - .grib2  — raw GRIB message (single variable, byte-range extracted)
//!   - .json   — metadata regenerated by cfgrib from the same .grib2 bytes
//!
//! 14/16 tests use full parity (1% relative tolerance on checkpoint values).
//! 2 NBM tests use degraded parity (aggregate stats only) due to DRT 3
//! spatial differencing decoder producing different individual grid point
//! values than eccodes, while aggregate statistics match exactly.
//!
//! Note: these test Rust grib crate vs Python cfgrib on identical bytes.
//! They do NOT test against the full Herbie acquisition pipeline, which
//! may select different variables or apply derivations (e.g., ECMWF snod).
//! See tile_integration.rs for full pipeline tests.
